...
import { useGetAllFilesQuery, useGetAllDatesQuery } from '../redux/files/filesApi';
...
const _Table1 = () => {
  ...
  // const { data: dates, error, isError, isLoading, isSuccess } = useGetAllDatesQuery({cam1: 'cam1_1', cam2: 'cam1_2'});
  // if (isSuccess) {
  // console.log('111')
  // setTablePagination(Math.ceil(dates.length / PAGE_SIZE));
  // setTableParams({
  //   pagination: {
  //     // current: 1,
  //     // pageSize: PAGE_SIZE,
  //     total: Math.ceil(dates.length / PAGE_SIZE)
  //   }
  // });
  // }
  // const allDates: string[] = [];
  // dates?.forEach((item) => {
  // allDates.push(new Date(item.dateTime).toLocaleDateString('fr-CA'));
  // });
